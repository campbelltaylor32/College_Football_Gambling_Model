...
fin_data %>%
  write.csv(file = paste0("CFB_Gambling_Results_2025_",week_update,".csv"), row.names = FALSE)
...
tot_talent_full %>% 
  write.csv(
    file = "CFB_Team_Talent_Data_2025.csv", row.names = FALSE
  )
...
tot_coaches_stat %>% 
  write.csv(file = "Coaches_Winning_CFB_2025.csv", row.names = FALSE)
####
...
tot_stats1 %>% 
  write.csv(
    file = "Game_Stats_Averages_CFB_PBP_Added_2025.csv", 
    row.names = FALSE
  )
...
tot_return %>% 
  write.csv(
    file = "Returning_Production_CFB_2025.csv", 
    row.names = FALSE
  )
